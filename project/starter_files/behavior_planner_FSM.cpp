...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // Our look_ahead_distance is the distance required to reach v_f from v_t using a gentle accel/deccel a
  // The calc_distance function, which we previously created, is included as a helper function in velocity_profile_generator.cpp to calculate look_ahead_distance
  // It can be used to meet the requirements
  //Create instance of VelocityProfileGenerator
  VelocityProfileGenerator vpg;
  //access calc_distance function using instance
  ...
  // The smallest distance is the most suitable value that allows us to meet the requirements  
  look_ahead_distance =
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      // At the stopping point, the vehicle should no longer be moving. Thus, our goal speed is 0.0
      goal.velocity.x = 0.0;
      goal.velocity.y = 0.0;
      goal.velocity.z = 0.0;
      ...
      // _speed_limit * std::sin/cos (goal.rotation.yaw);
    ...
    //if (utils::magnitude(ego_state.velocity) <=
        ...
        // Now that we know we are close or at the
